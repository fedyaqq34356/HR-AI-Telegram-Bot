...
KNOWLEDGE_KEYWORDS = {
    ...
    # ВАЖНО: Разделяем "запустить эфир" и "как сидеть в эфире"
...
def find_relevant_knowledge(question, user_lang='ru'):
    ...
    # ПРИОРИТЕТ 1: Точное совпадение по специфичным ключевым словам
    # Проверяем СНАЧАЛА более специфичные категории
    specific_checks = [
    ...
    # Если нашли специфичное совпадение - возвращаем только его
    if relevant:
        return relevant
    
    # ПРИОРИТЕТ 2: Общие категории
...
async def build_context_prompt(user_id, question, is_in_groups=False):
    ...
    # ВСЕГДА используем язык вопроса для ответа
    answer_lang = question_lang
    
    # Получаем релевантные знания из базы НА ЯЗЫКЕ ВОПРОСА
...
# Остальной код без изменений (check_faq_direct_match, is_contextual_question, get_ai_response_with_retry, get_ai_response)
# Копируем весь остальной код из исходного файла...
