...
аккаунт блокируется без возможности восстановления."""
    ...
    if any(kw in q_lower for kw in app_download_keywords):
        return 'Попробуй зайти через другой браузер или очисти кэш. Если не помогает — перезагрузи телефон и попробуй ещё раз. Если проблема останется — напиши мне, разберёмся! 😊'
