...
def calculate_dislike_ratio(dislikes, likes):
    """Считает коэффициент дизлайков"""
    total = dislikes + likes
    if total == 0:
        return 0.0
    return round(dislikes / total, 2)
...
📊 Расчет:
{dislikes} дизлайков ÷ ({dislikes} + {likes}) = {ratio}
...
📊 Розрахунок:
{dislikes} дизлайків ÷ ({dislikes} + {likes}) = {ratio}
...
📊 Calculation:
{dislikes} dislikes ÷ ({dislikes} + {likes}) = {ratio}
