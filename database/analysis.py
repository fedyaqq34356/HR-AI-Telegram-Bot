...
async def get_all_analysis_texts():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute('SELECT * FROM analysis_texts ORDER BY timestamp DESC') as cursor:
            return await cursor.fetchall()

async def get_all_analysis_audios():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute('SELECT * FROM analysis_audios ORDER BY timestamp DESC') as cursor:
            return await cursor.fetchall()

async def get_all_analysis_videos():
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute('SELECT * FROM analysis_videos ORDER BY timestamp DESC') as cursor:
            return await cursor.fetchall()