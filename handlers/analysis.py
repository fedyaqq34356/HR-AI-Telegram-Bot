...
async def cmd_start_analysis(message: Message, bot):
    ...
    await message.answer("🔍 Начинаю анализ сохраненных сообщений из группы...")
    
    messages = await get_unprocessed_messages()
    
    if not messages:
        await message.answer("❌ Нет новых сообщений для обработки. Бот должен быть добавлен в группу для автоматического сбора сообщений.")
        return
